...
RUN CGO_ENABLED=0 go build -o /watchledger-web ./cmd/web \
 ...
 && CGO_ENABLED=0 go build -o /watchledger-seed ./cmd/seedcatalogue \
 && CGO_ENABLED=0 go build -o /watchledger-sync ./cmd/syncprod

FROM alpine:3.20
RUN apk add --no-cache ca-certificates bash
WORKDIR /app
COPY --from=builder /watchledger-web /watchledger-engine /watchledger-ingest /watchledger-reproduce /watchledger-alerts /watchledger-seed /watchledger-sync /usr/local/bin/
